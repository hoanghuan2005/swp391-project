...
import { BookOpen, Download, Eye, FileText, Heart, MessageCircle, X } from "lucide-react";
...
const HOMEPAGE_WELCOME_MESSAGE = {
  id: "homepage-welcome",
  role: "ASSISTANT",
  content: "Hi, I am StudyMate AI. Ask me about study planning, document discovery, or any topic you are learning.",
};
...
export default function Homepage() {
  ...
  const [chatMessages, setChatMessages] = useState(() => [HOMEPAGE_WELCOME_MESSAGE]);
  ...
  const handleSendChatMessage = useCallback(async (message) => {
    ...
    try {
      ...
      const response = await askAi({ conversationId, message, mode: "HOMEPAGE_ASSISTANT" });
      ...
      setChatMessages((prev) => [...prev, { id: `error-${Date.now()}`, role: "ASSISTANT", content: "Sorry, please try again." }]);
    } finally {
  ...
  const filteredDocuments = useMemo(() => {
    return documents.filter((doc) => {
      ...
      const matchesCourse = filterData.course ? doc.course?.code?.toLowerCase().includes(filterData.course.toLowerCase()) || doc.course?.name?.toLowerCase().includes(filterData.course.toLowerCase()) : true;
      const matchesCategory = filterData.category ? doc.category?.toLowerCase() === filterData.category.toLowerCase() : true;
      
      // Sửa lỗi trắng trang khi backend trả về school null
      const docSchool = doc.schoolCode || doc.school?.code || doc.course?.schoolCode || doc.school || "";
      const matchesSchool = filterData.school 
        ? docSchool.toLowerCase().includes(filterData.school.toLowerCase()) || filterData.school.toLowerCase().includes(docSchool.toLowerCase())
        : true;
    ...
    <>
      <main className="flex-1">
        <div className="flex items-center justify-between mb-8">
          <h2 className="text-[26px] font-bold text-slate-800 tracking-tight font-sans">ShareDocs — Knowledge Base</h2>
        </div>

        <RecentDocuments />
        <UploadDocumentDialog open={uploadOpen} onOpenChange={setUploadOpen} onUploadSuccess={(newDoc) => upsertPublicDocument(newDoc)} />

        <section className="mb-10">
          <h3 className="text-xl font-bold mb-4 text-slate-800 tracking-tight">Explore Public Documents</h3>
          {isLoading ? (
            <div className="text-center text-slate-500 font-medium">Loading documents...</div>
          ) : filteredDocuments.length === 0 ? (
            <div className="text-center bg-slate-50 rounded-2xl text-slate-500 border border-slate-100 p-8">No documents found matching your criteria.</div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
              {filteredDocuments.map((doc) => {
                const isFavorited = favoritedIds.includes(doc.id); // Check xem file đã thích chưa
                return (
                  <Card key={doc.id} className="shadow-sm border-slate-100 hover:shadow-md transition-all group flex flex-col h-full rounded-[20px] overflow-hidden bg-white">
                    <CardContent className="p-4 flex-1 flex flex-col">
                      <div className="w-full aspect-[4/3] bg-slate-50 rounded-xl mb-3 -mt-4 border border-slate-200 group-hover:border-[#f26522]/20 flex items-center justify-center text-slate-300"><FileText className="w-12 h-12" /></div>
