...
import { Clock, FileText, ChevronRight, BookOpen, Heart, Eye, Download } from "lucide-react";
    ...
    <section className="mb-10" aria-labelledby="recent-docs-title">
      <div className="flex items-center justify-between mb-4 px-1">
        <div className="flex items-center gap-2">
          ...
          <h2 id="recent-docs-title" className="text-xl font-bold text-slate-800 tracking-tight">
            Recently Viewed
          </h2>
        </div>
      ...
      <ScrollArea className="w-full whitespace-nowrap rounded-xl">
        <div className="flex w-max space-x-4 p-1 pb-4">
          {loading ? (
            ...
            documents.map((doc) => (
              <div key={doc.id} className="w-[280px]">
                <Card
                  className="shadow-sm border-slate-100 hover:shadow-md transition-all group flex flex-col h-full rounded-[20px] overflow-hidden bg-white ring-0"
                >
                  <CardContent className="p-4 flex-1 flex flex-col">
                    ...
                    <CardDescription className="text-xs text-slate-500 font-medium mb-3 flex items-center gap-1.5 whitespace-normal">
                      <BookOpen className="w-3.5 h-3.5 flex-shrink-0" />{" "}
                      <span className="line-clamp-1">{doc.course?.code || "General"}</span>
                    </CardDescription>

                    <div className="text-[11px] text-slate-400 -mt-1 flex justify-between items-center">
                      <span>{formatTimeAgo(doc.lastViewedAt)}</span>
                      <span>{doc.downloadCount || 0} downloads</span>
                    </div>
                  </CardContent>

                  {/* Khu vực nút bấm */}
                  <CardFooter className="-mt-3 px-4 py-3 flex gap-2 border-t-0">
                    <Button
                      variant="outline"
